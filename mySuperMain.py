...
def getLastIndexToMyProfiles():
    folder_names = [name for name in os.listdir(cf.local['directory']) if
                    os.path.isdir(os.path.join(cf.local['directory'], name))]
...
def main(boite):
    ...
    driver.get(
        "https://accounts.google.com/v3/signin/identifier?continue=https%3A%2F%2Fmail.google.com%2Fmail%2F&ifkv=AS5LTAQD9a4oPjkvuTPriRcxp7d3vsx5qwDEoo9KM8o4nSzzSQRK24p_e4FG3TDhgAB0-88UY3Eaow&rip=1&sacu=1&service=mail&flowName=GlifWebSignIn&flowEntry=ServiceLogin&dsh=S940119914%3A1719931494548591&ddm=0")
    ...
    driver.get(cf.video_url_youtube)
    time.sleep(5)
    try:
        btnSubscribe = driver.find_elements(By.CLASS_NAME, cf.btnSubscribe)[4]
        btnSubscribe.click()
    except:
        pass
    time.sleep(2)
    driver.execute_script(f"document.getElementsByClassName('{cf.btnSubscribe}')[17].click()")
    time.sleep(cf.video_time-5)
    driver.execute_script(f"document.getElementsByClassName('{cf.video}')[0].click()")
    driver.execute_script("window.scrollTo({top: 500, behavior: 'smooth'});")
    time.sleep(3)
    len_commentaires = len(cf.commentaires)
    random_commentaire = cf.commentaires[random.randrange(len_commentaires)] 
    driver.execute_script("document.getElementById('placeholder-area').click()")
    time.sleep(2)
    driver.execute_script("document.getElementById('creation-box').setAttribute('class', 'focused style-scope ytd-commentbox style-scope ytd-commentbox')")
    driver.execute_script("document.getElementById('labelAndInputContainer').setAttribute('class', 'input-content focused style-scope tp-yt-paper-input-container')")
    driver.execute_script("document.getElementsByClassName('yt-spec-button-shape-next yt-spec-button-shape-next--filled yt-spec-button-shape-next--disabled yt-spec-button-shape-next--size-m')[0].setAttribute('class', 'yt-spec-button-shape-next yt-spec-button-shape-next--filled yt-spec-button-shape-next--call-to-action yt-spec-button-shape-next--size-m')")
    driver.execute_script(f"document.getElementById('contenteditable-root').textContent = '{random_commentaire}'")
    driver.execute_script("""
        const input = document.getElementById('contenteditable-root');
        if (input) {
